...
export function SystemMessage({ content }: SystemMessageProps) {
  ...
  return (
    <div className="flex justify-center my-1">
      <span className={`px-4 py-1.5 text-xs text-gray-500 dark:text-[#8696A0] bg-white/70 dark:bg-[#182229] rounded-lg shadow-sm text-center max-w-[85%] ${pinned ? 'not-italic flex items-center gap-1.5 font-medium' : 'italic'}`}>
        {pinned && <PinIcon />}
        {content}
      </span>
    </div>
